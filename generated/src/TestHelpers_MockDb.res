...
@genType
and entities = {
  @as("Gauge") gauge: entityStoreOperations<Types.gaugeEntity>,
  @as("LatestETHPrice") latestETHPrice: entityStoreOperations<Types.latestETHPriceEntity>,
  @as("LiquidityPool") liquidityPool: entityStoreOperations<Types.liquidityPoolEntity>,
  ...
  liquidityPoolUserMapping: entityStoreOperations<Types.liquidityPoolUserMappingEntity>,
  @as("LiquidityPoolWeeklySnapshot")
  liquidityPoolWeeklySnapshot: entityStoreOperations<Types.liquidityPoolWeeklySnapshotEntity>,
  @as("StateStore") stateStore: entityStoreOperations<Types.stateStoreEntity>,
...
let rec makeWithInMemoryStore = (inMemoryStore: IO.InMemoryStore.t) => {
  ...
  let entities = {
    gauge: {
      module(IO.InMemoryStore.Gauge)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.gauge,
        ~getKey=({id}) => id,
      )
    },
    latestETHPrice: {
      module(IO.InMemoryStore.LatestETHPrice)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.latestETHPrice,
        ~getKey=({id}) => id,
      )
    },
    liquidityPool: {
      module(IO.InMemoryStore.LiquidityPool)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.liquidityPool,
        ~getKey=({id}) => id,
      )
    },
    ...
    liquidityPoolUserMapping: {
      module(IO.InMemoryStore.LiquidityPoolUserMapping)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.liquidityPoolUserMapping,
        ~getKey=({id}) => id,
      )
    },
    ...
    stateStore: {
      module(IO.InMemoryStore.StateStore)->makeStoreOperator(
        ~inMemoryStore,
        ~makeMockDb=makeWithInMemoryStore,
        ~getStore=db => db.stateStore,
        ~getKey=({id}) => id,
      )
    },
    token: {
...
let executeMockDbLoadLayer = (
  ...
  let entityExecutors = [
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.gauge.get,
      ~inMemStoreSetFn=IO.InMemoryStore.Gauge.set,
      ~store=inMemoryStore.gauge,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.latestETHPriceIdsToLoad,
      ~dbReadFn=mockDb.entities.latestETHPrice.get,
      ~inMemStoreSetFn=IO.InMemoryStore.LatestETHPrice.set,
      ~store=inMemoryStore.latestETHPrice,
      ~getEntiyId=entity => entity.id,
    ),
    makeMockDbEntityExecuter(
      ~idsToLoad=loadLayer.liquidityPoolIdsToLoad,
      ~dbReadFn=mockDb.entities.liquidityPool.get,
      ~inMemStoreSetFn=IO.InMemoryStore.LiquidityPool.set,
    ...
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.liquidityPoolUserMapping.get,
      ~inMemStoreSetFn=IO.InMemoryStore.LiquidityPoolUserMapping.set,
    ...
    makeMockDbEntityExecuter(
      ...
      ~dbReadFn=mockDb.entities.stateStore.get,
      ~inMemStoreSetFn=IO.InMemoryStore.StateStore.set,
...
let writeFromMemoryStore = (mockDb: t, ~inMemoryStore: IO.InMemoryStore.t) => {
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.gauge},
    ~getRows=IO.InMemoryStore.Gauge.values,
    ~setFunction=IO.InMemoryStore.Gauge.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.latestETHPrice},
    ~getRows=IO.InMemoryStore.LatestETHPrice.values,
    ~setFunction=IO.InMemoryStore.LatestETHPrice.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.liquidityPool},
    ~getRows=IO.InMemoryStore.LiquidityPool.values,
    ~setFunction=IO.InMemoryStore.LiquidityPool.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.liquidityPoolUserMapping},
    ~getRows=IO.InMemoryStore.LiquidityPoolUserMapping.values,
    ~setFunction=IO.InMemoryStore.LiquidityPoolUserMapping.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
  ...
  mockDb->executeRows(
    ~inMemoryStore,
    ~getStore=self => {self.stateStore},
    ~getRows=IO.InMemoryStore.StateStore.values,
    ~setFunction=IO.InMemoryStore.StateStore.set,
    ~getKey=entity => entity.id,
  )
  mockDb->executeRows(
    ~inMemoryStore,
