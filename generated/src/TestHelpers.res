  ...
  let makeEventProcessor = (
    ...
    ({event, mockDb, ?chainId}) => {
      ...
      let asyncGetters: Context.entityGetters = {
        ...
        getLiquidityPoolNew: async id =>
          mockDbClone.entities.liquidityPoolNew.get(id)->Belt.Option.mapWithDefault([], entity => [
            entity,
          ]),
...
module VotingReward = {
  module NotifyReward = {
    ...
    let processEvent = EventFunctions.makeSyncEventProcessor(
      ...
      ~eventName=Types.VotingReward_NotifyReward,
    )
    ...
    @genType
    let createMockEvent = args => {
      let {?from, ?reward, ?epoch, ?amount, ?mockEventData} = args
      ...
      let params: Types.VotingRewardContract.NotifyRewardEvent.eventArgs = {
        ...
        reward: reward->Belt.Option.getWithDefault(Ethers.Addresses.defaultAddress),
        epoch: epoch->Belt.Option.getWithDefault(Ethers.BigInt.zero),
        amount: amount->Belt.Option.getWithDefault(Ethers.BigInt.zero),
      }
